...
<%
    ExtendedUser editUser = (ExtendedUser) request.getAttribute("editUser");
    if (editUser == null) {
        response.sendRedirect("userlist.jsp");
        return;
    }

    String error = (String) session.getAttribute("editUserError");
...
<body>
    ...
    <div class="container mt-4">
        ...
        <form action="EditUserServlet" method="POST" class="needs-validation" novalidate>
            ...
            <div class="row mb-3">
                <div class="col-md-6">
                    ...
                    <input type="text" class="form-control" id="firstName" name="firstName" 
                           value="<%= editUser.getFirstName() %>" required>
                </div>
                <div class="col-md-6">
            ...
            <div class="row mb-3">
                ...
                <div class="col-md-6">
                    ...
                    <input type="tel" class="form-control" id="phone" name="phone" 
                           value="<%= editUser.getPhone() %>" required>
                </div>
            </div>
                ...
                <div class="col-md-6">
                    <div class="form-check">
                        <input class="form-check-input" type="checkbox" id="isStaff" name="isStaff" 
                               value="true" <%= editUser.isStaff() ? "checked" : "" %>>
                        <label class="form-check-label" for="isStaff">
                            Staff Member
                        </label>
                    </div>
                </div>
                <div class="col-md-6 staff-fields" style="display: <%= editUser.isStaff() ? "block" : "none" %>;">
                    <div class="row">
                        <div class="col">
                            ...
                            <input type="number" class="form-control" id="staffCardId" name="staffCardId" 
                                   value="<%= editUser.isStaff() ? editUser.getStaffCardId() : "" %>">
                        </div>
                        <div class="col">
                            <div class="form-check mt-4">
                                <input class="form-check-input" type="checkbox" id="isAdmin" name="isAdmin" 
                                       value="true" <%= editUser.isAdmin() ? "checked" : "" %>>
                    ...
                    <div class="row mt-3">
                        <div class="col">
                            <div class="staff-password-section" style="display: none;">
                                ...
                                <small class="form-text text-muted">Required when adding staff role</small>
                            </div>
                            <div class="admin-password-section" style="display: none;">
                                ...
                                <small class="form-text text-muted">Required when adding admin role</small>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="row mb-3">
                <div class="col-md-6">
                    <div class="form-check">
                        <input class="form-check-input" type="checkbox" id="isCustomer" name="isCustomer" 
                               value="true" <%= editUser.isCustomer() ? "checked" : "" %>>
                ...
                <div class="col-md-6">
                    <div class="customer-type-section" style="display: <%= editUser.isCustomer() ? "block" : "none" %>;">
                        <label for="customerType" class="form-label">Customer Type</label>
                        <select class="form-select" id="customerType" name="customerType">
                            <option value="INDIVIDUAL" <%= editUser.isCustomer() && editUser.getCustomerType() == Customer.Type.INDIVIDUAL ? "selected" : "" %>>Individual</option>
                            <option value="COMPANY" <%= editUser.isCustomer() && editUser.getCustomerType() == Customer.Type.COMPANY ? "selected" : "" %>>Company</option>
                        </select>
                    </div>
                </div>
            </div>
    ...
    <script>
        const wasStaff = <%= editUser.isStaff() %>;
        const wasAdmin = <%= editUser.isAdmin() %>;
        ...
        function updatePasswordFields() {
            ...
            // Hide both password sections initially
            staffPasswordSection.style.display = 'none';
            adminPasswordSection.style.display = 'none';
            
            if (!wasStaff && isStaffChecked) {
                // If becoming staff (but not admin), show staff password
                if (!isAdminChecked) {
                    staffPasswordSection.style.display = 'block';
                }
            }
            
            if (isAdminChecked && !wasAdmin) {
                // If becoming admin, show admin password (regardless of previous staff status)
                adminPasswordSection.style.display = 'block';
                staffPasswordSection.style.display = 'none'; // Hide staff password if shown
            }
        }

        // Make staff and customer checkboxes mutually exclusive
        document.getElementById('isStaff').addEventListener('change', function() {
            ...
            // If unchecking staff, also uncheck admin
            if (!this.checked) {
        ...
        // Toggle customer fields visibility and handle mutual exclusivity
